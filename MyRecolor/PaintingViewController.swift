...
class PaintingViewController: UIViewController {
    ...
    @IBAction func taptap(sender: UITapGestureRecognizer) {
        ...
        print(point)
        imageView.buckerFill(point, replacementColor: UIColor(red: 1, green: 0, blue: 0, alpha: 1))
    }
...
extension UIImageView {
    func buckerFill(touchPoint:CGPoint, replacementColor:UIColor){
        ...
        let targetColor = colorAtPoint(touchPoint, inData: dataType)
        let stack = Stack()
        stack.push(touchPoint)
        while(!stack.isEmpty()){
            let point = stack.pop()!
